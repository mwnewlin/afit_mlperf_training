...
class SubmissionReport(object):
    ...
    def print_results(self):
        result_columns = RESULT_SUBM_META_COLUMNS + \
            RESULT_ENTRY_META_COLUMNS + BENCHMARK_NAMES

        print("\n" + "RESULTS\n" + "-------\n")
        print(",".join(result_columns))
        for entry_name in self.results:
            value_list = [self.results[entry_name][key]
                          for key in result_columns]
            value_list = [
                "-" if val is None else str(val) for val in value_list]
