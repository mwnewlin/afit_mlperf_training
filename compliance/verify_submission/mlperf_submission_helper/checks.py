    ...
    def name_in(self, path, ref_list):
        ...
        if basename in ref_list:
            self.report.add_passed_check(
                '{} name is in {}.'.format(path, ref_list))
        else:
            self.report.add_failed_check(
                '{} name not in {}.'.format(path, ref_list))
        ...
        try:
            for entry_name in os.listdir(result_root_dir):
                ...
                for result_name in os.listdir(entry_dir):
                    ...
                    result_num = constants.REQUIRED_RESULT_NUM.get(
                        result_name, 0)
                    for i in range(result_num):
                        ...
                        division = self.result_entry_meta[entry_name].get(
                            'division')
            ...
            self.report.add_error(
                'Unable to verify results dir: {}'.format(str(e)))
