...
parser.add_argument('--start_epoch', default=-1, type=int,
                    help='Number of epochs at which to start from')
...
def main():
    ...
    loss_results, cer_results, wer_results = torch.Tensor(
        params.epochs), torch.Tensor(params.epochs), torch.Tensor(params.epochs)
    best_wer = None
    try:
    ...
    if args.continue_from:
        ...
        # Python index start at 0 for training
        start_epoch = int(package.get('epoch', 1)) - 1
        start_iter = package.get('iteration', None)
        if start_iter is None:
            # Assume that we saved a model after an epoch finished, so start at the next epoch.
            start_epoch += 1
            start_iter = 0
        else:
            start_iter += 1
        ...
        loss_results[:start_epoch], cer_results[:start_epoch], wer_results[:start_epoch] = package['loss_results'][:
                                                                                                                   start_epoch], package['cer_results'][:start_epoch], package['wer_results'][:start_epoch]
        print(loss_results)
        epoch = start_epoch
    ...
    for epoch in range(start_epoch, params.epochs):
        ...
        for i, (data) in enumerate(train_loader, start=start_iter):
            ...
            sizes = Variable(input_percentages.mul_(
                int(seq_length)).int(), requires_grad=False)
