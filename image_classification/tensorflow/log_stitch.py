...
DEFERRED_PREFIX = re.compile(
    "{} \[({})\].+".format(_COMMON_PATTERN, _UUID_PATTERN))
...
def main():
    ...
    for line in deferred_evaluations:
        ...
        logged_eval_info = re.sub(
            "{} \[{}\]".format(_COMMON_PATTERN, key), "", line)
        # print(logged_eval_info, DEFERRED_INFO_PATTERN.match(logged_eval_info))
        eval_timestamp, eval_value = DEFERRED_INFO_PATTERN.match(
            logged_eval_info).groups()
