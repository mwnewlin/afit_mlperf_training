...
def evaluate(prev_model, cur_model, readouts=200, verbose=1, resign_threshold=0.95):
    ...
    cur_win_pct = main.evaluate_evenly(prev_model_save_path, cur_model_save_path,
                                       game_output_dir, readouts=readouts, games=goparams.EVAL_GAMES_PER_SIDE)
    ...
    qmeas.record('eval_summary', {
                 'win_pct': cur_win_pct, 'model': cur_model, 'keep': keep})
    return keep
...
def bury_latest_model():
    ...
    cmd = 'mv {}* {}/'.format(os.path.join(SELFPLAY_DIR,
                                           model_name), BURY_SELFPLAY_DIR)
    # delete any selfplay games from that model too
    ...
    for suffix in suffixes:
        cmd = 'cp {} {}'.format(prev_save_file + suffix,
                                new_save_file + suffix)
        print('DBUG ', cmd)
        if os.system(cmd) != 0:
...
def rl_loop():
    ...
    if goparams.EVALUATE_PUZZLES:
        ...
        result, total_pct = predict_games.report_for_puzzles(
        ...
        qmeas.record('puzzle_summary', {'results': repr(
            result), 'total_pct': total_pct, 'model': new_model})
        qmeas._flush()
