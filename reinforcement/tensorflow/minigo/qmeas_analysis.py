...
def selfplay_stats(jsons):
    ...
    print('Average Selfplay Time: {:.2f}s'.format(
        sum(sp_times) / len(sp_times)))
...
def proporition_times(jsons):
    ...
    if last is not None:
        ...
        for name in reversed(sorted(tvals, key=lambda n: tvals[n])):
            secs = tvals[name]
            print('{:<20}{:7.1f}%{:>30.1f}'.format(
                name, secs / max(tvals.values()) * 100, secs))
...
def puzzle_quality(jsons, lowest_wall):
    ...
    for j in jsons:
        ...
        if j['name'] != 'puzzle_total':
            continue
        print('{:f}\t{:.1f}%'.format(
            (j['wall_time'] - lowest_wall) / 60 / 60, 100 * float(j['value'])))
...
def puzzle_results(jsons):
    ...
    for j in jsons:
        ...
        for puz in sorted(res):
            ...
            print('{:>2.0f}%    {}'.format(
                correct_count * 100.0 / len(res[puz]), puz))
...
def improved_factor(jsons, lowest_wall):
    ...
    for j in jsons:
        ...
        if res['keep']:
            ...
            print('  {:<20}    {:>5.1f}  (+{:3.1f}%)  {:3.0f}h'.format(
                res['model'], math.log(fact, 10), res['win_pct'] * 100, hours))
...
def main():
    ...
    print()
    print()
    print('Total Duration: {:.2f} hours'.format(
        (biggest_wall_time - lowest_wall_time) / 60 / 60))
    print('Last update: {:.2f} hours ago'.format(
        (time.time() - biggest_wall_time) / 60 / 60))
