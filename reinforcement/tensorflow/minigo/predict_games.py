...
def main():
    ...
    for model_path in models:
        model_results[model_path], total_pct = report_for_puzzles(
            model_path, sgf_files, 2, tries_per_move=1)
    ...
    for attempt in range(rounds):
        for filename in sgf_files:
            if filename not in results:
                results[filename] = []
            move_ratings = predict_move(
                filename, network, tries_per_move=tries_per_move)
...
def predict_9x9_puzzles(model_path, tries_per_puzzle):
    ...
    result, total_pct = report_for_puzzles(
        model_path, sgf_files, tries_per_puzzle)
...
def predict_move(filename, network, tries_per_move=1, readouts=1000):
    ...
    for position_w_context in replay:
        ...
        for i in range(tries_per_move):
            move, correct_move, is_correct = predict_position(
                position_w_context, player, readouts=readouts)
            if is_correct:
                num_correct += 1
